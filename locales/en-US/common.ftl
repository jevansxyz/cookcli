# Navigation
nav-recipes = Recipes
nav-shopping-list = Shopping List
nav-pantry = Pantry
nav-preferences = Preferences

# Search
search-placeholder = Search recipes...
search-no-results = No recipes found

# Common Actions
action-add = Add
action-remove = Remove
action-edit = Edit
action-save = Save
action-cancel = Cancel
action-back = Back
action-delete = Delete
action-clear = Clear
action-print = Print
action-preview = Preview

# Common Labels
label-scale = Scale
label-servings = Servings
label-time = Time
label-difficulty = Difficulty
label-name = Name
label-description = Description

# Editor
editor-unsaved-changes = Unsaved changes
editor-saved = Saved
editor-saving = Saving...
editor-save-failed = Save failed
editor-placeholder = Enter your recipe here...

# LSP Status
lsp-connected = LSP Connected
lsp-disconnected = Disconnected
lsp-error = LSP Error

# New Recipe
new-recipe = New Recipe
new-recipe-path = Recipe path
new-recipe-filename = Recipe name
new-recipe-directory = Recipe directory
new-recipe-directory-placeholder = e.g. Dinner/Italian
new-recipe-directory-hint = Leave empty to save in the root folder
new-recipe-name = Recipe name
new-recipe-placeholder = Pasta Carbonara
new-recipe-hint = The file name of the recipe
new-recipe-create = Create Recipe
new-recipe-url = Recipe URL (optional)
new-recipe-url-placeholder = https://...
new-recipe-url-hint = Paste a recipe URL to import and convert with Claude AI (metric units)

# Delete Recipe
delete-recipe = Delete Recipe
delete-recipe-confirm = Are you sure you want to delete this recipe?
delete-recipe-warning = This action cannot be undone.

# Errors
error-title = Something went wrong
error-back-home = Back to recipes
